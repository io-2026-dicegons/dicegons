
from game.squad import Squad
from game.squad_type import SquadType

class Army:

    def __init__(self):
        self.first_squad = None
        self.second_squad = None

    def check_can_buy_unit(self, squad_type):
        if self.first_squad == None :
            return True
        
        if self.second_squad == None :
            return True
        
        if self.first_squad.squad_type == squad_type.id_unit_type and self.first_squad.quantity < squad_type.max_quantity :
            return True
        
        if self.second_squad.squad_type == squad_type.id_unit_type and self.second_squad.quantity < squad_type.max_quantity :
            return True
        
        return False
    
    def buy_unit(self, unit_type_id, squad_type):

        if self.first_squad != None and self.first_squad.squad_type == squad_type.id_unit_type and self.first_squad.quantity < squad_type.max_quantity :
            self.first_squad.quantity = self.first_squad.quantity + 1
            return True
        
        if self.second_squad != None and self.second_squad.squad_type == squad_type.id_unit_type and self.second_squad.quantity < squad_type.max_quantity :
            self.second_squad.quantity = self.second_squad.quantity + 1
            return True
        
        if self.first_squad == None :
            self.first_squad = Squad(0,unit_type_id,1)
            return True
        
        if self.second_squad == None :
            self.second_squad = Squad(0,unit_type_id,1)
            return True
        
        return False

    def get_squad(self, squad_nr ):

        if squad_nr == 1:
            return self.first_squad
        elif squad_nr == 2:
            return self.second_squad
        
    def set_squad(self, squad_nr, squad ):

        if squad_nr == 1:
            self.first_squad = squad;
        elif squad_nr == 2:
            self.second_squad = squad;